...
+ 從領域 ( Domain ) 的脈絡 ( Context ) 中抽除術語 ( Term ) 與描述 ( Statement )
+ 由術語與描述構成統一語言 ( Ubiquitous Language )
+ 經由統一語言來描述問題，並將其抽象出對應處理的模組 ( Model )
+ 當模組過多，設定脈絡邊界 ( Bounded Context ) 避免重疊的行為與應用範圍

需要注意，對於問題解析或模組化並沒有一定的步驟，但 DDD 對於模組抽離，並將問題側重在模組的疊帶改善，其目的應是『領域模組與軟體技術解耦』，避免『商業邏輯與軟體底層技術綁定』從而導致的維護性降低。
